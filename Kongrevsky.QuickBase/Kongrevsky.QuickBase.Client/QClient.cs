...
namespace Intuit.QuickBase.Client
{
    ...
    public class QClient : IQClient
    {
        ...
        public IQApplication Connect(string applicationId, string token, List<string> loadTables = null)
        {
            Exception exception = new Exception("Can't connect to QuickBase");
            for (int i = 0; i < 10; i++)
            {
                try
                {
                    var qApplication = ApplicationFactory.CreateInstance(this, applicationId, token, loadTables);
                    return qApplication;
                }
                catch (Exception e)
                {
